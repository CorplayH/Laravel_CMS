...
@section('content')
    <div class="card">
        <form class="mt-3" action="{{route ('news.customMenu.update',$customMenu)}}" method="post">
            @csrf @method('PUT')
            <div class="card-body">
                ...
                <div class="form-group mt-3">
                    ...
                    <input type="text" name="cname" value="{{$customMenu['cname']}}" class="form-control" placeholder="">
                </div>
                    ...
                    <div class="input-group mb-3">
                        <select class="custom-select" name="parentId">
                            ...
                            @foreach($allCategories as $category)
                                <option value="{{$category['id']}}" {{$category['_disable']}}
                                        @if($category['id'] == $customMenu['parentId']) selected @endif>
                                    {!! $category['_cname'] !!}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
